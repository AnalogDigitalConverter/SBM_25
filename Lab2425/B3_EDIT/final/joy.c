...
void control_press_tmr_Callback(void *args);
...
void joy_Thread (void *argument){
  ...
  if(init_Joystick_mbed() && !Init_MsgQueue_joy())
  {
    ...
    id_long_press_tmr = osTimerNew(control_press_tmr_Callback, osTimerPeriodic, (void *)0, NULL);

    /*fsm in operation*/
    while(1)
    {
      flags = osThreadFlagsWait((EXTI_JOY_IRQ_FLAG | BOUNCE_TIMEOUT_FLAG | SEND_FLAG), osFlagsWaitAny, osWaitForever);

      if (flags & EXTI_JOY_IRQ_FLAG)
      {
        osTimerStart(id_rebotes_tmr, BOUNCE_TIMER_VALUE);
      }

      else if (flags & BOUNCE_TIMEOUT_FLAG)
      {
        //Long-Press Control (LONG_PRESS_TIMER_VALUE), defined as 1000U (1s); see at define section
        osTimerStart(id_long_press_tmr, CONTROL_PRESS_TIMER_VALUE);
      }

      else
      {
        osTimerStop(id_long_press_tmr);
...
void control_press_tmr_Callback(void *args)
{
	if(cnt == 20) //condicion larga
	{
      joy_msg.long_press = 1;
      osThreadFlagsSet(id_joy_Thread, SEND_FLAG);
	}
	else if (check_pins())
    {
      cnt++;
    }
    else //condicion corta, levantado
    {
