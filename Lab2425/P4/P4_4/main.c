...
static uint16_t cursor_upper, cursor_lower = 0;

//P4_4
static const uint32_t valor_1 = 1234;
static const float valor_2 = 3.14159;
  
//P4_4
static char linea1[32];
static char linea2[32];
...
//P4_4
void LCD_print(char line1[32], char line2[32]);

//P4_1
void LCD_symbolToLocalBuffer_L1(uint8_t symbol);
void LCD_upper_buffer_write(char input_string[]);

//P4_3
void LCD_symbolToLocalBuffer_L2(uint8_t symbol);
void LCD_lower_buffer_write(char input_string[]);
...
int main(void)
{
  ...
  */
  ...
  //P4_4: Create subfunction
	snprintf(linea1, sizeof(linea1), "Prueba valor1: %d", valor_1);
	snprintf(linea2, sizeof(linea2), "Prueba valor2: %g", valor_2);
...
void LCD_symbolToLocalBuffer_L1(uint8_t symbol)
{
  ...
  for(i = 0; i<12; i++)
  {
	...
	buffer [i + cursor_upper] = value1;			//posicion 1 en buffer 1
	buffer [i + 128 + cursor_upper] = value2;	//posicion 2 en buffer 2
  }

  cursor_upper = cursor_upper + Arial12x12 [offset];
}
...
void LCD_symbolToLocalBuffer_L2(uint8_t symbol)
{
  ...
  for(i = 0; i<12; i++)
  {
	...
	buffer [i + 256 + cursor_lower] = value1;	//posicion 1 en buffer 1
	buffer [i + 384 + cursor_lower] = value2;	//posicion 2 en buffer 2
  }
  cursor_lower = cursor_lower + Arial12x12 [offset];
}
...
void LCD_lower_buffer_write(char input_string[])
{
  ...
  {
	symbol = input_string[i];
	symbolToLocalBuffer(2, symbol);
  }
}

void LCD_print(char line1[32], char line2[32])
{
  LCD_upper_buffer_write(line1);
  LCD_lower_buffer_write(line2);
