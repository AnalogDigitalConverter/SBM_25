...
/* Private functions ---------------------------------------------------------*/
static void initTimer_TIM2_CH4(void);
static void initTimer_TIM3_CH1(void);
static void initPin_PB11_Output(void);
static void initPin_PB4_input(void);
static void init_B1USER_Button(void);
...
static void initTimer_TIM2_CH4(void){
	...
	/* EXTERNALIZADO PÚBLICO, MODIFICADO POR INTERRUPCIONES HW (stm32f4xx_it.c)*/
			/* uint16_t periodo_tim2;											*/
			/* TIM_HandleTypeDef htim2;										*/
			/* TIM_OC_InitTypeDef TIM_Channel_InitStruct;	*/
	
	/*HAL Clock Channelling / Enable*/
	__HAL_RCC_TIM2_CLK_ENABLE();		//Enable htim2 Perif
	
	/*Timer Register Config*/
	htim2.Instance = TIM2;
	periodo_tim2 = 41999;
	htim2.Init.Prescaler = 0;								//82MHz -> APB Timer (/2) = 42Mhz
	htim2.Init.Period = periodo_tim2;						//42Mhz / 42000 = 1KHz
...
static void initTimer_TIM3_CH1(void){
	...
	/*Declarado Private TypeDef
	TIM_IC_InitTypeDef sConfigIC;*/
	
	htim3.Instance = TIM3;
	//SI VIENE DEL APB1 -> 84MHz
	htim3.Init.Prescaler 			= 1; 	//Div 2 48MH
...
static void initPin_PB4_input(void){
	  ...
	  /*Declarados en Main o Previamente*/
