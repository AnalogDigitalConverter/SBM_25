...
int init_Th_lcd(void){
	...
	//id_MsgQueue_lcd = osMessageQueueNew(MSGQUEUE_OBJECTS_LCD, sizeof(MSGQUEUE_OBJ_LCD), NULL); mirar como es mas eficiente
...
osMessageQueueId_t get_id_MsgQueue_lcd(){
return id_MsgQueue_lcd;
}
static void Th_lcd(void *argument){ // funcion del hilo
  MSGQUEUE_OBJ_LCD msg_lcd;
	osStatus_t status;
	bool up=false, down=false, left=false, rigth=false, center=false;
  while(1){
    status = osMessageQueueGet(get_id_MsgQueue_lcd(), &msg_lcd, NULL, 100);
		if(status==osOK){
			up     = msg_lcd.dir & (1 << 0);
			down   = msg_lcd.dir & (1 << 1);
			left   = msg_lcd.dir & (1 << 2);
			rigth  = msg_lcd.dir & (1 << 3);
			center = msg_lcd.dir & (1 << 4);
			
			if(up)         {dataToBuffer("UP","+++");}
			else if(down)  {dataToBuffer("DOWN","+++");}
			else if(left)  {dataToBuffer("LEFT","+++");}
			else if(rigth) {dataToBuffer("RIGTH","+++");}
			else if(center){dataToBuffer("CENTER","+++");}
