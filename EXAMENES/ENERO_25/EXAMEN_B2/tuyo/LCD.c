...
osThreadId_t tid_ThLCD_test;                        // thread id
 
void ThLCD_test (void *argument);                   // thread function
 
int Init_ThLCD_test(void) {
 
  tid_ThLCD_test = osThreadNew(ThLCD_test, NULL, NULL);
  if (tid_ThLCD_test == NULL) {
    return(-1);
  }
...
void ThLCD_test (void *argument) {
 
  while (1) {
		
		sprintf(LCD_recibida.data_1, "Linea 1");
		sprintf(LCD_recibida.data_2, "Linea 2");
		osMessageQueuePut(get_id_MsgQueue_LCD(), &LCD_recibida, 0U, 0U);
		osDelay(1000);
		LCD_Clean();
		osDelay(1000);
  }
