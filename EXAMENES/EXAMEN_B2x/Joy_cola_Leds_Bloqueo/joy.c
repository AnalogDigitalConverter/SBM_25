...
//Halt
static osTimerId_t id_halt_tmr;
void halt_tmr_Callback(void *args);
...
void joy_Thread (void *argument){
  ...
  if(init_Joystick_mbed() && !Init_MsgQueue_joy()){
    ...
    id_long_press_tmr = osTimerNew(long_press_tmr_Callback, osTimerOnce, (void *)0, NULL);
		id_halt_tmr 			= osTimerNew(halt_tmr_Callback, osTimerOnce, (void *)0, NULL);

    /*fsm in operation*/
    while(1)
		{	
			flags = osThreadFlagsWait((EXTI_JOY_IRQ_FLAG | BOUNCE_TIMEOUT_FLAG | HALT_FLAG), osFlagsWaitAny, osWaitForever);
			...
			if (flags & BOUNCE_TIMEOUT_FLAG)
			{
				...
				osTimerStart(id_halt_tmr, HALT_TIMER_VALUE);
...
void halt_tmr_Callback(void *args)
{
	osThreadFlagsSet(id_joy_Thread, HALT_FLAG);
}
